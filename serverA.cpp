...
int main (int argc , char * argv[]){
    ...
    //These following codes was influenced by tutorials on youtube.

    // filling in the information of the server.
    memset(&server,0,length);
    server.sin_family = AF_INET;
    server.sin_port = htons(A_PORT_NUM);
    ...
    cout<<"server A is up and running using UDP on port " << ntohs(server.sin_port)<<endl;
    ...
    while (1){
        ...
        numberOfElmts = dataLength/ sizeof(long signed int);

        cout << "The Server A has received "<< numberOfElmts << " numbers"<<endl;
      ...
      // adding the numbers onto a vector for processing.
        ...
        cout << "The Server A has successfully finished the reduction " << comndString << ": "<<result << endl;
